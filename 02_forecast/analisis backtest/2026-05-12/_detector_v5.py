"""
Detector v5 - elimina las 2 reglas que generaban falsos positivos en v4:

  Cambio 1: PROMO_2X_DESPLOME (lift <=0.3) -> NO alertar
    Razon en v4: -3,147 unidades de dano (174 SKUs)
    Causa: baseline_8w inflado por Easter/Dia del Trabajador.

  Cambio 2: SATURACION en W2 (mecanicas 12X/6X/4X/3X) -> NO alertar
    Razon en v4: -684 unidades dano agregado (12X_W2 -446, 3X_W2 -238, etc.)
    Causa: factor 0.3-0.5 demasiado agresivo cuando motor ya predecia con cuidado.

Mantiene SATURACION en W3+ (que SI funciono).
"""
import pandas as pd
import numpy as np
import re
...
OUT_XLSX   = r"c:\Users\sanhu\Mi unidad\Proyectos\Desarrollo\Odoo Produccion\analisis backtest\2026-05-12\_detector_v5_output.xlsx"
...
# ----------------------------------------------------------------------
print("=" * 110)
print("DETECTOR v5 - elimina PROMO_2X_DESPLOME y SATURACION_W2 (ruido confirmado en v4)")
print("=" * 110)
    ...
    if has_promo and not has_cambio:
        ...
        if mecanica == "2X":
            if lift >= 2.5:
                ...
                # v5: NO alertar 2X cuando lift no es extremo al alza
                # (eliminado PROMO_2X_DESPLOME con lift<=0.3, era ruido)
                return None
        ...
        if mecanica in ("12X", "6X", "4X", "3X"):
            if weeks_active <= 1 and lift >= 1.5:
                ...
                # v5: SOLO alertar saturacion en W3+ (eliminado W2 que era ruido)
                factor = max(0.6, lift)
                tipo = f"SATURACION_{mecanica}_W{weeks_active}"
            else:
                # v5: NO alertar SATURACION_W2 (lift <=0.5 con weeks_active 2-4)
                # ni casos no claros
                return None
            return {
