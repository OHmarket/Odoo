# ============================================================
# OH Quiebre de Stock - Deteccion de quiebres POR EVIDENCIA (v3.3)
# ============================================================
#
...
#
# Principio (canon SAP / Oracle Retail):
...
#
# --- v3.3 (2026-06-21): reconstruccion FORWARD para backfill historico largo ---
# El modo backward (v3.2) ancla en el stock.quant de HOY y rola hacia atras.
# A pocos dias es exacto (cron incremental); a 12-18 meses acumula drift y el
# guard `reliable` corta el tramo viejo -> 2024 sub-marcado. NO es falta de dato
# (stock_move tiene historia completa desde 2023-05): es la DIRECCION del roll.
#
# Modo 'forward' (gated por context recon='forward'):
#   - Acumula desde el inicio de la historia hacia adelante. Balance de apertura
#     a date_from = Σ (entradas - salidas) de los moves locales ANTES de date_from.
#   - Sin drift de ancla-hoy: el error no crece con la distancia al presente.
#   - Confiabilidad por RECONCILIACION (no por guard de negativos): la suma
#     forward hasta HOY debe dar el stock.quant actual. Por SKU, si
#     |balance_forward(hoy) - quant_hoy| <= RECON_TOL -> confiable (2024 incluido);
#     si no -> sub-marca (preserva la asimetria: ante drift, false negative).
#   - Solo valido con date_to == hoy (backfill). El cron incremental sigue
#     usando 'backward' (default) -> comportamiento productivo INALTERADO.
#   - Telemetria recon_ok / recon_fail: fraccion de SKUs que reconcilian; mirar
#     ANTES de confiar en el historico reconstruido.
#
# Logica por (sala, SKU, dia D). Orden: stock-primero, gate solo arbitra ceros.
#   end_raw   = balance reconstruido (SIN pisar -> mantiene aritmetica)
...
#   reliable   = backward: tramo reciente sin cruce a negativo (corta la cola).
#                forward:  el SKU entero reconcilia vs stock real de hoy.
...
# === v3.3: metodo de reconstruccion ===
RECON_DEFAULT     = 'backward'   # 'backward' (cron incremental, INALTERADO) | 'forward' (backfill historico)
RECON_TOL_DEFAULT = 1.0          # tolerancia de reconciliacion forward (unidades)

# Parametros default (overridable via context)
TAIL_WINDOW_DAYS_DEFAULT   = 7                 # cola de re-calculo en modo incremental
BACKFILL_FLOOR_DEFAULT     = (2024, 1, 1)      # piso historico
...
# === Override ONE-OFF para backfill historico forward ===
# El boton "Ejecutar" de la UI NO propaga el context custom, asi que para la
# corrida historica DESCOMENTA la linea de abajo, ejecuta UNA vez, y vuelve a
# comentarla (asi el cron diario sigue en incremental/backward, intacto).
# CTX = dict(CTX); CTX['mode'] = 'backfill'; CTX['recon'] = 'forward'
...
# === v3.3: metodo de reconstruccion (default backward -> cron INALTERADO) ===
RECON     = (str(CTX.get('recon') or RECON_DEFAULT)).lower()
RECON_TOL = float(CTX.get('recon_tol', RECON_TOL_DEFAULT))
...
if not env.cr.fetchone()[0]:
    ...
    if not team_to_warehouses:
        ...
        _log_info('STOCKOUT %s: mode=%s recon=%s rango=[%s..%s] tail=%s N_activo=%s run_id=%s teams=%s',
                  VERSION_ID, mode, RECON, date_from, date_to, tail_window, ACTIVE_WINDOW_DAYS,
                  run_id, sorted(team_to_warehouses.keys()))

        if RECON == 'forward' and date_to != date_today:
            _log_warn('STOCKOUT %s: recon=forward pero date_to=%s != hoy=%s; la '
                      'reconciliacion contra stock.quant NO aplica (resultado dudoso).',
                      VERSION_ID, date_to, date_today)
        ...
        if not internal_location_ids:
            ...
            # 4c) v3.3 FORWARD: balance de apertura a date_from
            #     (Σ entradas - salidas de moves con fecha LOCAL < date_from, desde el inicio).
            #     Solo se calcula en modo forward; el modo backward no lo usa.
            opening_qty = {}
            if RECON == 'forward':
                env.cr.execute("""
                    SELECT location_id, location_dest_id, product_id, SUM(product_qty) AS qty
                    FROM stock_move
                    WHERE state = 'done'
                      AND (date AT TIME ZONE 'UTC' AT TIME ZONE %(tz)s)::date < %(date_from)s
                      AND (location_id = ANY(%(locs)s) OR location_dest_id = ANY(%(locs)s))
                    GROUP BY location_id, location_dest_id, product_id
                """, {'tz': TZ_NAME, 'date_from': date_from, 'locs': internal_location_ids})
                for loc_id, dest_id, pid, qty in env.cr.fetchall():
                    qty = float(qty or 0.0)
                    pid = int(pid)
                    src_wh = location_to_warehouse.get(loc_id)
                    dst_wh = location_to_warehouse.get(dest_id)
                    if src_wh is not None and dst_wh != src_wh:
                        k = (src_wh, pid)
                        opening_qty[k] = opening_qty.get(k, 0.0) - qty
                    if dst_wh is not None and dst_wh != src_wh:
                        k = (dst_wh, pid)
                        opening_qty[k] = opening_qty.get(k, 0.0) + qty
            ...
            for (wh, pid, _) in movements.keys():
                if pid in sellable_pids:
                    relevant.add((wh, pid))
            # forward: incluir tambien pares que solo aparecen en la apertura
            if RECON == 'forward':
                for (wh, pid) in opening_qty.keys():
                    if pid in sellable_pids:
                        relevant.add((wh, pid))
            ...
            # 6) Reconstruccion + deteccion por evidencia
            ...
            recon_ok = 0            # v3.3 forward: SKUs que reconcilian vs stock real de hoy
            recon_fail = 0          # v3.3 forward: SKUs que NO reconcilian (sub-marcados)
            stockout_by_abcxyz = {}
            ...
            for (wh_id, pid) in relevant:
                ...
                if RECON == 'forward':
                    # acumula desde la apertura hacia adelante (sin drift de ancla-hoy)
                    bal = opening_qty.get((wh_id, pid), 0.0)
                    fi = 0
                    while fi < n_days:
                        mv = movements.get((wh_id, pid, days[fi]))
                        qin  = mv['in']  if mv else 0.0
                        qout = mv['out'] if mv else 0.0
                        bal = bal + qin - qout
                        balance_end_arr[fi] = bal
                        fi += 1
                    # confiabilidad = reconcilia vs stock real de HOY (valido si date_to==hoy)
                    resid = balance_end_arr[n_days - 1] - current_qty.get((wh_id, pid), 0.0)
                    if resid < 0:
                        resid = -resid
                    sku_ok = (resid <= RECON_TOL)
                    reliable = [sku_ok] * n_days
                    if sku_ok:
                        recon_ok += 1
                    else:
                        recon_fail += 1
                else:
                    # backward (metodo v3.2, sin cambios): ancla en HOY, rola atras
                    balance_end_arr[n_days - 1] = current_qty.get((wh_id, pid), 0.0)
            ...
            recon_total = recon_ok + recon_fail
            recon_rate = (float(recon_ok) / recon_total) if recon_total else None

            result = {
                'version': VERSION_ID, 'mode': mode, 'recon': RECON, 'run_id': run_id,
                'date_from': str(date_from), 'date_to': str(date_to),
                'date_today': str(date_today), 'tail_window_days': tail_window,
                'active_window_days': ACTIVE_WINDOW_DAYS, 'recon_tol': RECON_TOL,
                ...
                'recon_ok': recon_ok, 'recon_fail': recon_fail, 'recon_rate': recon_rate,
...
# ============================================================
# Notificacion visible del resultado (popup tras la corrida manual)
# `result` queda definido en todas las ramas (lock / no_wh / no_loc / main).
# ============================================================
def _fmt_result(r):
    if r.get('aborted'):
        return 'ABORTADO: %s' % r.get('reason')
    parts = [
        'mode=%s recon=%s' % (r.get('mode'), r.get('recon')),
        'rango %s..%s' % (r.get('date_from'), r.get('date_to')),
        'dias-quiebre creados=%s (full=%s, parcial=%s)' % (
            r.get('records_created'), r.get('stockout_days_full'),
            r.get('stockout_days_partial')),
        'pares SKUxsala=%s' % r.get('wh_product_pairs'),
    ]
    if r.get('recon') == 'forward':
        rate = r.get('recon_rate')
        rate_s = ('%.1f%%' % (rate * 100.0)) if rate is not None else 'n/a'
        parts.append('RECONCILIACION: %s ok / %s fail -> %s' % (
            r.get('recon_ok'), r.get('recon_fail'), rate_s))
    return ' | '.join(parts)
...
action = {
    ...
    'params': {
        'title': 'Quiebre de Stock %s' % VERSION_ID,
        'message': _fmt_result(result),
        'type': 'warning' if result.get('aborted') else 'success',
        'sticky': True,
    },
}
