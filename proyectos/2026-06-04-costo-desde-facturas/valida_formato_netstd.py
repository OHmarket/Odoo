"""valida_formato_netstd.py — READ-ONLY.
Igual que valida_formato pero cuenta UNIDADES por net/std (robusto a UoM mentirosa
'PAR'/'Display de N'), no por qty*pack. Recalibra tabla (formato,cc) y valida la
prediccion del recargo total en facturas 100% cubiertas.
"""
import sys
import re
...
def main():
    ...
    for m in moves:
        ...
        for l in lines:
            ...
            if std <= 0 or ccn <= 0 or fmt == '(vacio)' or net <= 0:
                plist.append(None)
                continue
            unid = net / std                      # <-- unidades por costo, robusto a UoM
            plist.append({'fmt': fmt, 'cc': ccn, 'unid': unid, 'net': net})
        if rec > 0 and plist:
    ...
    print("tabla (formato,cc): %s entradas | facturas 100%% cubiertas: %s\n" % (len(table), full))
    if ratios:
        ratios.sort()
        m = mean(ratios)
        print("PREDICCION recargo total (predicho/real), UNIDADES=net/std (%s facturas):" % len(ratios))
        ...
        print("  dentro de 0.9-1.1: %.0f%%  (antes con qty*pack era 55%%)" % (100.0*sum(1 for x in ratios if 0.9 <= x <= 1.1)/len(ratios)))
